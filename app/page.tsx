...
export default function Page() {
  ...
  useEffect(() => {
    axios
      .get('https://web-production-f58e4.up.railway.app/predict') // ✅ Updated to Railway API
      .then((res) => {
              ...
              {last10.length > 0 && (
                <div className="mb-4 text-center text-white text-sm">
                  <span className="bg-red-600 text-white px-4 py-2 rounded shadow-lg font-semibold">
                    Current Time: {formatTimestamp(
                      new Date(new Date(last10[0].timestamp).getTime() - 3600000).toISOString()
                    )
                    }
                  </span>
                </div>
